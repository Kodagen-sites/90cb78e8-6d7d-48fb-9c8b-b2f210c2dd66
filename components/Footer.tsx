    ...
    <ul className="space-y-3 text-sm text-white/70">
      {email && (
        <li className="flex items-start gap-2">
          <Mail size={14} className="mt-0.5 text-cream/80 flex-shrink-0" />
          <a href={`mailto:${email}`} className="hover:text-white">{email}</a>
        </li>
      )}
      {phone && (
        <li className="flex items-start gap-2">
          <Phone size={14} className="mt-0.5 text-cream/80 flex-shrink-0" />
          <a href={`tel:${phone}`} className="hover:text-white">{phone}</a>
        </li>
      )}
      {location && (
        <li className="flex items-start gap-2">
          <MapPin size={14} className="mt-0.5 text-cream/80 flex-shrink-0" />
          <span>{location}</span>
        </li>
      )}
    </ul>
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <footer className="relative z-20 bg-contrast border-t border-white/10">
    ...
    <Shell>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 pb-12 border-b border-white/10">
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 self-start rounded-full bg-cream px-6 py-3 text-sm font-medium text-ink hover:opacity-90 transition"
        >
